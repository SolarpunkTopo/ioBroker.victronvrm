    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (response.status === 200 && response.data.token && response.data.idUser) {
                ...
                this.adapter.log.info('BearerToken erfolgreich geholt');
                
                // Erfolgreich den API-Token und idUser zurückgeben
                resolve({ BearerToken, idUser });
            } else {
                this.adapter.log.error('Fehler beim Abrufen des BearerToken: ' + response.status);
                reject(new Error('Invalid response'));
            }
        } catch (error) {
            this.adapter.log.error('Fehler beim API-Token-Abruf: ' + error.message);
            reject(error);
        }
    });
 }
...
async getInstallationId(BearerToken, idUser) {
		...
		try {
            ...
            if (data.records && data.records.length > 0) {
                ...
                this.adapter.log.info(`Installationen erfolgreich abgerufen: ${installations.map(i => `${i.id}: ${i.name}`).join(', ')}`);
            ...
            this.adapter.log.error(`Fehler beim Abrufen der Installation-ID: ${error.message}`);
			this.adapter.log.error(`Fehler beim Abrufen der Installation-ID: ${error.message}`);
            throw error;
        }
    }
...
async getLongTermApiToken(BearerToken, idUser) {
    try {
        // Erstelle die Konfigurationsoptionen für den API-Request
        const options = {
            method: 'POST',
            ...
            data: { name: 'iobrokerapitoken' } // Daten für den API-Request
        };

        // Mache den API-Request und warte auf das Ergebnis
        const response = await this.axios.request(options);

        // Rückgabe des erhaltenen Tokens
        return response.data.token;
    } catch (error) {
        // Fehlerbehandlung und Ausgabe im Adapter-Log
        this.adapter.log.warn(JSON.stringify(error));
        throw new Error('Fehler beim Abrufen des Long-Term API Tokens');
    }
}
...
getAuthorizationHeader() {
    ...
    this.adapter.log.debug('ApiToken wird verwendet: ' + JSON.stringify(this.adapter.config.VrmApiToken));
	} else if (this.adapter.BearerToken) {
...
/**
    ...
    startPolling(BearerToken, installationIds, interval) {
        const pollData = async (installationId, installationName) => {
            const url = `https://vrmapi.victronenergy.com/v2/installations/${installationId}/diagnostics?count=1`;
