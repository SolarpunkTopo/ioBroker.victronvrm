...
class WebhookClient {
    ...
    startPollingWebhook(id, customSettings, interval) {
        ...
        // Verwende die spezifische Webhook-URL für den Datenpunkt oder die globale URL
        const webhookUrl = customSettings.webhookUrl || this.adapter.config.webhookUrl;
        const getVariableName = customSettings.getVariableName || 'value';

        if (!webhookUrl) {
            this.adapter.log.error(`Keine Webhook-URL konfiguriert für Datenpunkt ${id} und keine globale Webhook-URL verfügbar.`);
            return;
        }
        ...
        this.webhookIntervals[id] = setInterval(async () => {
            try {
                ...
                if (state && state.val != null) {
                    ...
                    this.adapter.log.debug(`Webhook-Request erfolgreich für ${id}: Status ${response.status}`);
                } else {
