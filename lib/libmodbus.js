...
class ModbusClient {
    ...
    /**
     * Stoppt das Polling für einen spezifischen Datenpunkt.
     * @param {string} datapoint - Der Datenpunkt-ID in ioBroker.
     */
    stopPollingForDatapoint(datapoint) {
        const key = datapoint;
        ...
        if (this.pollingIntervals[key]) {
            ...
            this.adapter.log.info(`Polling für Datenpunkt ${datapoint} gestoppt.`);
        }

        // Schließe den Modbus-Client
        if (this.modbusClients[key]) {
            this.modbusClients[key].close(() => {
                this.adapter.log.info(`Modbus-Client für Datenpunkt ${datapoint} geschlossen.`);
            });
